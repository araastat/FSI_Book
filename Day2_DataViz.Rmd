...
```{r Day2-DataViz-1, include=FALSE}
...
```{r Day2-DataViz-2, eval=T, echo=T}
library(ggplot2)
...
```{r Day2-DataViz-3, eval=T, echo=T}
...
```{r Day2-DataViz-4, eval=T, echo=F}
ggplot(mtcars, aes(x = wt, y = mpg)) + 
  geom_point()+ 
  geom_smooth()+
  labs(x = 'Weight', 
...
```{r Day2-DataViz-12, warning=F, message=FALSE}
library(forecast)
...
```
```{r Day2-DataViz-20, eval=T, echo=F}
dos %>% filter(str_detect(Recipient_Location, '^C')) %>%
ggplot(aes(x = year(as_date(Award_Start_Date)), 
           y = Award_Transaction_Value, 
           color = Recipient_Location, 
...
```{r Day2-DataViz-22, echo = F, out.height="600pt", fig.align='center'}
library(choroplethr)
data(df_pop_county)
...
```{r Day2-DataViz-24, echo = T, eval=T}
...
```
```{r Day2-DataViz-25, eval=F, fig.align='center', out.height="300pt"}
## library(ggplot2)
## library(plotly)
